...
const LoginForm = ({ onFinish, isLoading }) => {
  ...
  useEffect(() => {
    const savedIdentifier = localStorage.getItem(
      STORAGE_KEYS.REMEMBER_IDENTIFIER,
    );
    if (savedIdentifier) {
  ...
  const validateIdentifier = (value) => {
    ...
    const isUsername =
      usernameRegex.test(value) && value.length >= 3 && value.length <= 50;
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8">
        <Form.Item name="remember" valuePropName="checked" noStyle>
          <Checkbox className="text-gray-500 font-medium">Remember me</Checkbox>
        </Form.Item>
        <Link
          to="/forgot-password"
          className="text-blue-600 hover:text-blue-700 font-semibold text-sm transition-all cursor-pointer"
        >
          Forgot password?
        </Link>
      </div>
