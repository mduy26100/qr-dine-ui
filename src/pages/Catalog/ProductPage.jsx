...
import { Button, Card, Typography, Space, message, Modal, Segmented, Input, Select, TreeSelect, Divider } from 'antd';
...
export const ProductPage = () => {
    ...
    const handleAvailableChange = useCallback((value) => {
        setSearchParams((params) => {
            if (value !== '') {
    ...
    const hasActiveFilters = searchTermParam || categoryIdParam || isAvailableParam;
    ...
    const memoizedTableParams = useMemo(() => {
        ...
        return params;
    }, [tableState.pageNumber, tableState.pageSize, searchTermParam, categoryIdParam, isAvailableParam]);

    const { 
        data: pageResponse, 
        isLoading: isPageLoading, 
        refetch: refetchPage 
    } = useGetMyProductsByPage(memoizedTableParams, { enabled: viewMode === 'table' });

    const [cursorState, setCursorState] = useState({ limit: 10, cursorCreatedAt: null, cursorId: null });
    ...
    const memoizedCursorParams = useMemo(() => {
        ...
        if (cursorState.cursorCreatedAt) params.cursorCreatedAt = cursorState.cursorCreatedAt;
        if (cursorState.cursorId) params.cursorId = cursorState.cursorId;
        if (searchTermParam) params.searchTerm = searchTermParam;
        if (categoryIdParam) params.categoryId = categoryIdParam;
        if (isAvailableParam !== '') params.isAvailable = isAvailableParam;
        return params;
    }, [cursorState.limit, cursorState.cursorCreatedAt, cursorState.cursorId, searchTermParam, categoryIdParam, isAvailableParam]);
    ...
    } = useGetMyProductsByCursor(memoizedCursorParams, { enabled: viewMode === 'list' });
    ...
    useEffect(() => {
        ...
        if (incomingData.length > 0) {
            setAccumulatedList(prev => {
                ...
                const newItems = incomingData.filter(item => !prev.some(p => p.id === item.id));
    ...
    const safePageData = pageResponse?.items ? pageResponse : (pageResponse?.data || {});
    ...
    const handleOpenCreate = useCallback(() => {
        messageApi.open({ type: 'info', content: 'Tính năng thêm sản phẩm đang phát triển' });
    }, [messageApi]);

    const handleOpenEdit = useCallback((record) => {
        messageApi.open({ type: 'info', content: `Đang sửa: ${record.name}` });
    }, [messageApi]);
    ...
    const handleDelete = useCallback((record) => {
        modal.confirm({
            ...
            centered: true,
            onOk: async () => {
                messageApi.open({ type: 'info', content: 'Tính năng xóa đang được phát triển' });
            },
        });
    }, [modal, messageApi]);
    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <Title level={2} className="!mb-1">Danh sách sản phẩm</Title>
                ...
                <div className="flex flex-wrap items-center justify-end gap-3">
                    ...
                    <Button 
                        icon={<ArrowPathIcon className={`w-4 h-4 ${(isPageLoading || isCursorLoading) ? 'animate-spin' : ''}`} />} 
